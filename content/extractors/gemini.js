...
export default {
  ...
  extract() {
    ...
    userTurns = getOutermostElements('user-query, .user-query-text, .user-query-container, [data-test-id*="user-query"]');
    aiTurns = getOutermostElements('model-response, .model-response-text, .model-response-container, message-content, [data-test-id*="model-response"]');
    ...
    // Strategy 4: message containers by class pattern inside main
    ...
    if (turns.length >= 2) {
      ...
      const validTurns = turns.map((el, i) => ({
